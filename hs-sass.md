...
提供形態	① Webアプリ (Next.js + Supabase Auth)② REST/GraphQL API + Function Calling (JSON Schema)③ Shopify / WooCommerce アドオン
課金	月額基本料 + API従量／PDF出力従量（従量課金は Stripe Metered Billing）
非機能	平均応答 3 秒以下, 推論精度> 85 %（Top-3内）, HS改訂反映< 72 h, ISMS相当のログ保全5年
...
ロール	代表ユースケース
通関担当者	① 商品名・仕様を貼り付け → HS候補＋根拠表示② NACCS CSV (輸入･輸出) をダウンロード／税関へ電子申請
